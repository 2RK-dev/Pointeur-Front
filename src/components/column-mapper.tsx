...
import type { TableColumn, ColumnMapping } from "@/lib/types"
...
export function ColumnMapper({
  ...
  const getMappedTableColumn = (fileColumn: string) => {
    return mappings.find((m) => m.fileColumn === fileColumn)?.tableColumn || null
  }
  ...
  const isRequired = (columnName: string | null) => {
    if (!columnName) return false
    return tableColumns.find((c) => c.name === columnName)?.required || false
  }

  const unmappedRequired = tableColumns.filter(
    (col) => col.required && !mappings.some((m) => m.tableColumn === col.name),
  )
  ...
  const getMappedColumns = () => {
    return mappings.filter((m) => {
      ...
      if (m.fileColumn === undefined || m.fileColumn === null) return false
      return true
    })
  }

  const getConfidence = (fileColumn: string) => {
    return mappings.find((m) => m.fileColumn === fileColumn)?.confidence || 0
  }
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {fileHeaders.map((fileHeader, index) => {
          ...
          const required = isRequired(mappedColumn)
          ...
          <div className="space-y-2">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Voici comment vos données seront importées (affichage des {previewData.length} premières lignes)
              </p>
            </div>
                    ...
                    {getMappedColumns().map((mapping) => (
                      <th key={mapping.tableColumn} className="text-left p-2 font-medium text-xs">
                        <div className="flex items-center gap-1.5">
                          ...
                          {isRequired(mapping.tableColumn) && (
                            <Badge variant="destructive" className="text-xs">
                              requis
                            </Badge>
                          )}
